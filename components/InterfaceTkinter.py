...
class FenetreRecherche(tk.Tk):
    def __init__(self):
        ...
        self.checkbox = ttk.Checkbutton(
            cadre, text="Recherche dans le titre uniquement", variable=self.checkbox_var
        )
        ...
        self.choix_categorie = ttk.Combobox(
            cadre,
            values=[
                "VEHICULES",
                "Voitures",
        ...
        self.rayon_slider = ttk.Scale(
            cadre,
            from_=0,
            to=200,
            variable=self.rayon_val,
        ...
        # Bouton de recherche
        bouton_rechercher = ttk.Button(
            cadre, text="Rechercher", command=self.rechercher
        )
        bouton_rechercher.grid(row=4, column=0, columnspan=2, pady=10)

        # Barre de chargement
        self.progress_bar = ttk.Progressbar(
            cadre, orient="horizontal", length=200, mode="indeterminate"
        )
    ...
    def rechercher(self):
        ...
        if verif(ville):
            print(
                f"Recherche en cours avec le terme '{recherche}' dans la ville '{ville}' dans la categorie '{choix_categorie}'"
            )
            ...
            print(
                f"Recherche en cours avec le terme '{recherche}' dans toute la France dans la categorie '{choix_categorie}'"
            )
